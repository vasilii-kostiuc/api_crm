...
class ModularProvider extends ServiceProvider {
    ...
    private function getApiRoutes(string $mod, string $sub, string $relativePath, string $path) {
    ...
    private function getMiddleware(string $mod, string $key = 'web') {
        $middleware = [];

        $config = config('modular.groupMiddleware');
        if (isset($config[$mod])) {
            if (array_key_exists($key, $config[$mod])) {
                $middleware = array_merge($middleware, $config['mod']);
            }
        }
        return $middleware;
    }
